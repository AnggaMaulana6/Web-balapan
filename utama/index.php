...
if(isset($_POST['submit'])){
    ...
    $sql1 = mysqli_query($con, "SELECT * FROM user WHERE nama = '$nama' AND password = '$password' AND status = 'aktif'");
    $numRow =mysqli_num_rows($sql1);
    $data = mysqli_fetch_assoc($sql1);

    if($numRow == 1){
        if ($data['level'] == 'admin') {
            $_SESSION['no_lambung'] = $data['no_lambung'];
                    ...
                    <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                        <div class="w-full">
                            ...
                            <label class="block text-sm">
                                <span class="text-gray-700 dark:text-gray-400">User Nama</span>
                                <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="nama" name="nama" required />
                            </label>
                            ...
                            <input type="submit" name="submit" value="Login" class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" />
